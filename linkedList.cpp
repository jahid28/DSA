...
void InsertAtMid(Node *&head,Node *&tail,int pos, int d)
{
    ...
    if(pos==1){ //only if we want to insert at 1st pos
        InsertAtHead(head,d);
        return;
    }
 ...
 if(temp->next==NULL){ //only if we want to insert at last pos
        InsertAtTail(tail,d);
        return;
    }
...
void deleteNode(int pos,Node*&head,Node*&tail){
    if(pos==1){ //if deleted node is first node
        Node* temp=head;
        head=head->next;
        temp->next=NULL;
        delete temp;
        ...
        if(curr->next==NULL){ //if deleted node is last node
        prev->next=NULL;
        tail=prev;
        curr->next=NULL;
        delete curr;
